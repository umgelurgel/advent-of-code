...
if __name__ == '__main__':
    ...
    if explore_map:
        ...
        # Try filling out the map with oxygen
        ...
        with open("15_b_input.txt", "r") as file:
            lines = file.read().split('\n')
        ...
        while True:
            ...
            print(counter)
            next_oxygens = deque()
            # process all current oxygen tiles
            while oxygens:
                # Get next oxgen tile
                x,y = oxygens.popleft()

                # Find neighbouring tiles that can be filed with oxygen
                if tiles[y+1][x] == '.':
                    tiles[y + 1][x] = 'O'
                    next_oxygens.append((x, y+1))
                if tiles[y-1][x] == '.':
                    tiles[y - 1][x] = 'O'
                    next_oxygens.append((x, y-1))
                if tiles[y][x+1] == '.':
                    tiles[y][x+1] = 'O'
                    next_oxygens.append((x+1, y))
                if tiles[y][x-1] == '.':
                    tiles[y][x-1] = 'O'
                    next_oxygens.append((x-1, y))

            print_map(tiles)
            oxygens = next_oxygens
            if not oxygens:
                break
